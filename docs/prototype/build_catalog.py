...
for genre in ("27", "35", "878", "16", "80"):                # horror, comedy, sci-fi, animation, crime
    sources.append(("/discover/movie", {"with_genres": genre, "sort_by": "vote_count.desc", "page": 1}))
...
films = dict(sorted(films.items(), key=lambda kv: -standing(kv[1]))[:220])
...
with ThreadPoolExecutor(max_workers=10) as pool:
    ...
    for fid, name in zip(ids, pool.map(director, ids)):
        if name:
            films[fid]["d"] = name
